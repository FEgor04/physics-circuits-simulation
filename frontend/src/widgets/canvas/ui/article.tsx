...
export function EducationArticleWidget() {
  ...
  return (
    <article className="prose m-auto mt-8 w-4/6 space-y-4 lg:prose-lg">
      ...
      <p>
        Ток, протекающий через <Latex>{"$R_1$"}</Latex> и <Latex>{"$R_2$"}</Latex> одинаков пока к выходу источнику
        постоянного тока ничего не подключено. А суммарное сопротивление пары резисторов при последовательном
        соединении:
      </p>
      <p>
      ...
      <p>
        ...
        компонентов, <Latex>{"$V_{out}$"}</Latex> будет меняться в зависимости от внешних условий, влияющих на датчик.
        Подключив это выходное напряжение к аналоговому входу Ардуино, можно получать информацию о температуре, уровне
        освещённости и других параметрах среды.
      </p>
      <p>
      ...
      <p>
        В этом случае <Latex>{"$V_{out}$"}</Latex> уже не может быть расcчитано лишь на основе значений{" "}
        <Latex>{"$V_{in}$"}</Latex>, <Latex>{"$R1$"}</Latex> и <Latex>{"$R_2$"}</Latex>: сама нагрузка провоцирует
      ...
      <p>
        ...
        чем больше номинал <Latex>{"$R_2$"}</Latex> по отношению к сопротивлению <Latex>{"$L$"}</Latex>. Чтобы
        нивелировать этот эффект мы могли бы использовать в качестве <Latex>{"$R_1$"}</Latex> и <Latex>{"$R_2$"}</Latex>{" "}
        резисторы, например, в 10 раз меньших номиналов.
      </p>
      <figure className="mx-auto h-80 w-80">
        <GetZoomCoefficientProvider zoomCoefficient={48}>
          <SelectComponentProvider selected={undefined} onSelect={() => {}}>
      ...
      <p>
        Если потребление тока нагрузкой неравномерно во времени, <Latex>{"$V_{out}$"}</Latex> также будет неравномерным.
      </p>
